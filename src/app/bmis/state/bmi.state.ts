...
import { AddBmi, DeleteBmi, LoadBmiById, LoadBmis, LoadDeletedBmis, UpdateBmi } from './bmi.actions';
                    ...
                    context.setState(
                        patch({
                            bmis: updateItem<IBmiModel>(
                                (toGet: IBmiModel | undefined) => toGet?.id === response.id, response
                            )
                        })
                    );
                } else {
    ...
    DeleteBmi(context: StateContext<BmiStateModel>, { id }: DeleteBmi) {
        return this.service.deleteBmi(id).pipe(
            tap(() => {
                context.setState(
                    patch({
                        bmis: removeItem<IBmiModel>(
                            (toDelete: IBmiModel | undefined) => toDelete?.id === id
                        ),
                    })
                );
                this.zone.run(() => { 
